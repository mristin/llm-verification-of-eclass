...
def main() -> int:
    ...
    if Step.DOCTEST in selects and Step.DOCTEST not in skips:
        ...
        for pth in (repo_root / "llm_verification_of_eclass").glob("**/*.py"):
    ...
    if (
        ...
        print(
            "Checking that llm_verification_of_eclass/__init__.py "
            "and pyproject.toml coincide..."
        )
        exit_code = call_and_report(
            verb=(
                "check that llm_verification_of_eclass/__init__.py "
                "and pyproject.toml coincide"
            ),
            cmd=[
                sys.executable,
        ...
        print(
            "Skipped checking that llm_verification_of_eclass/__init__.py "
            "and pyproject.toml coincide..."
        )

    # NOTE (mristin):
    # We need to check for the Python version since ``argparse`` output changes
    # between the versions. Hence, we pin it at the moment to Python 3.11.
    if (3, 11) <= sys.version_info < (3, 12):
        ...
        print(
            ...
            "since we pin it on Python version 3.11."
        )
